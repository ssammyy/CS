...
/**
 * Dialog component for editing existing purchase orders.
 * Provides a comprehensive form for updating PO details and line items.
 */
@Component({
  selector: 'app-edit-purchase-order-dialog',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
      ...
      <div class="flex items-center justify-between mb-6">
        <div>
          ...
          <p class="text-gray-600 mt-1">
            ...
            <span *ngIf="!loading">Update purchase order {{ data.purchaseOrder.poNumber }}</span>
          </p>
        </div>
        ...
        <div class="border-t border-gray-200 pt-6">
          <div class="flex items-center justify-between mb-4">
            <div>
              ...
              <p class="text-sm text-gray-600 mt-1">Manage products in this purchase order</p>
            </div>
            <app-accent-button 
              label="Add Item"
              icon="add"
...
export class EditPurchaseOrderDialogComponent implements OnInit {
  ...
  constructor(
    ...
    private dialogRef: MatDialogRef<EditPurchaseOrderDialogComponent>,
  ...
  populateForm(): void {
    ...
    // Clear existing line items and add current ones
    this.lineItemsArray.clear();
    this.purchaseOrder.lineItems.forEach(item => {
      this.lineItemsArray.push(this.createLineItemForm(item));
    });
  }

  createLineItemForm(item?: any): FormGroup {
  ...
  onSubmit(): void {
    if (this.purchaseOrderForm.valid) {
      ...
      this.purchaseOrdersService.updatePurchaseOrder(this.purchaseOrder.id, request, 'current-user').subscribe({
        next: (result) => {
